...
public class Vision extends SubsystemBase {
  ...
  private static final boolean BUMP_SUPPRESSION_ENABLED = false;
  ...
  public Command rotateToTower(DoubleSupplier vxSupplier, DoubleSupplier vySupplier) {
    return this.run(
            () -> {
              Pose2d robotPose = drivebase.getPose();
              Optional<Rotation2d> maybeHeading = getShuttleHeading();
              ...
              Logger.recordOutput("Vision/Tower/HeadingErrorDeg", Math.toDegrees(headingError));
              Logger.recordOutput("Vision/Tower/OmegaRadPerSec", omega);
              Logger.recordOutput("Vision/Tower/Active", true);
            })
        .finallyDo(
            () -> {
              drivebase.drive(new ChassisSpeeds(0, 0, 0));
              Logger.recordOutput("Vision/Tower/Active", false);
            });
  }
